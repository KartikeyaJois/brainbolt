...
func (r *UserCacheRepository) Get(userID int) (*models.User, error) {
...
func (r *UserCacheRepository) Set(userID int, user *models.User) error {
	key := userCacheKeyPrefix + strconv.Itoa(userID)
...
func (r *UserCacheRepository) QueueSet(pipe *redis.Pipeline, userID int, user *models.User) error {
	key := userCacheKeyPrefix + strconv.Itoa(userID)
	data, err := json.Marshal(user)
	if err != nil {
		return err
